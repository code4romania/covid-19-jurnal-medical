...
namespace StamAcasa.Api.Tests
{
    public class ProfileGetUserInfoBySubShould : ControllerBaseTest<ProfileController>
    {
        ...
        [Fact]
        public async Task Return_user_info_for_a_newly_created_user()
        {
        ...
        [InlineData(3)]
        [InlineData(1222)]
        public async Task Return_profile_when_request_is_made_for_family_member(int requestedId)
        {
            var user = GetClaimsPrincipal(new[] { new Claim("sub", "my-random-value") });
            SetUserInControllerContext(user);
        ...
        public async Task Not_allow_getting_profile_for_non_family_users(int? requestedId, int[] familyIds)
        {
            var user = GetClaimsPrincipal(new[] { new Claim("sub", "my-random-value") });
            SetUserInControllerContext(user);
